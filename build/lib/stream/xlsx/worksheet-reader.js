...
class WorksheetReader extends EventEmitter {
  ...
  parse() {
    ...
    return _wrapAsyncGenerator(function* () {
      ...
      if (_this2.workbook.hyperlinkRelationships && _this2.workbook.hyperlinkRelationships[_this2.id]) {
        _this2._hyperlinkRels = _this2.workbook.hyperlinkRelationships[_this2.id];
      }
                      ...
                      if (row && c) {
                        ...
                        if (hyperlinks) {
                          ...
                          if (hyperlink) {
                            ...
                            if (hyperlink.rId && _this2._hyperlinkRels && _this2._hyperlinkRels[hyperlink.rId]) {
                              const rel = _this2._hyperlinkRels[hyperlink.rId];
                        ...
                        if (_this2._dataTables.length > 0) {
                          ...
                          for (const dataTable of _this2._dataTables) {
                            ...
                            if (cellAddr.row >= range.top && cellAddr.row <= range.bottom && cellAddr.col >= range.left && cellAddr.col <= range.right && c.ref !== masterAddress // Don't overwrite master cell
                              ...
                              if (attributes.ref) cell.value.ref = attributes.ref;
